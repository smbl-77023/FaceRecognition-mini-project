...
names = ['Gia Mantegna', 'Nicole Gale Anderson', 'Samantha Barks', 'Sarah Hyland', 'Alexis Knapp', 'Alia Shawkat', 'Carly Schroeder', 'Diego Boneta',
         ...
         'Steven R. McQueen', 'Tori Black', 'Charlie McDermott', 'Chris Brown', 'Cody Horn', 'Cody Linley', 'Dakota Johnson', 'Danielle Panabaker','Bhaskar Nayak']
...
def trainResnet():
    ...
    textarea.delete('1.0', END)
    global model
    resnet = ResNet34(input_shape=(X_train.shape[1], X_train.shape[2], X_train.shape[3]), weights='imagenet', include_top=False)
    ...
    if os.path.exists("model/model_weights.hdf5") == False:
        ...
        hist = model.fit(X_train, y_train, batch_size = 16, epochs = 20, validation_data=(X_test, y_test), callbacks=[model_check_point], verbose=1)
    ...
    textarea.insert(END,"HaarCascade & ResNet34 Accuracy  : "+str(a)+"\n")
    textarea.insert(END,"HaarCascade & ResNet34 Precision : "+str(p)+"\n")
    textarea.insert(END,"HaarCascade & ResNet34 Recall    : "+str(r)+"\n")
    textarea.insert(END,"HaarCascade & ResNet34 FSCORE    : "+str(f)+"\n\n")        
